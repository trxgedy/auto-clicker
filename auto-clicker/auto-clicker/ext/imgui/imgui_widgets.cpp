...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (is_visible)
    {
        ...
        {
            window->DrawList->AddRectFilled( check_bb.Min, check_bb.Max, ImColor( 61, 133, 224 ), style.FrameRounding );
        }
    }
    ...
    if (is_visible && label_size.x > 0.0f)
        RenderText(label_pos + ImVec2( 5, 0 ), label );
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(grab_bb.Min - ImVec2( 8, -1 ), grab_bb.Max + ImVec2( -5, -1 ), GetColorU32( g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab ), style.GrabRounding );
    ...
    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Min.x + 2, frame_bb.Min.y + style.FramePadding.y - 27), label);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (render_cursor || render_selection)
    {
        ...
        {
            ImU32 col = GetColorU32( ImGuiCol_TextDisabled );
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(cursor_screen_rect.Min, cursor_screen_rect.GetBL(), GetColorU32( ImGuiCol_TextDisabled ));
    ...
    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Min.x + 2, frame_bb.Min.y - 25), label);
